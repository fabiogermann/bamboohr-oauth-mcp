...
| `OAUTH_ALLOWED_REDIRECT_URIS`     | yes      | —             | Comma-separated allowlist of MCP-client redirect URIs. `/authorize`, `/register`, and `/connect/callback` all reject any URI not in this list (open-redirect protection). For **Cursor** use exactly: `cursor://anysphere.cursor-mcp/oauth/callback,https://www.cursor.com/agents/mcp/oauth/callback`. Exact-string match, case-sensitive. `/register` filters the client's requested URIs against this list and echoes back the intersection (does not 400 if the client also sends URIs you don't allow). |
